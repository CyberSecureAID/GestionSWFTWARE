  ...
  document.getElementById('page-body').innerHTML=`
    ...
    <div class="tx-list" style="margin-bottom:24px">
      ${state.accounts.map(a=>`<div class="tx-item">
        ...
        ${state.accounts.length>1?actionMenu('acc-'+a.id,[{label:'Eliminar',icon:'ti-trash',onclick:`deleteAccount('${a.id}')`,danger:true}]):''}
      </div>`).join('')}
    </div>

    <div class="section-header"><div class="section-title help-row">Respaldo y almacenamiento ${helpBtn('cfg_about_file')}</div></div>
