...
function renderPage(){
  ...
  const titles={dashboard:'Dashboard',reports:'Reportes',personal:'Ingresos y Gastos',trade:'Capital / Trading',budgets:'Presupuestos',goals:'Metas de ahorro',business:navLabel('business','Ventas y Gastos'),purchases:navLabel('purchases','Compras'),inventory:navLabel('inventory','Inventario'),settings:'Configuración'};
  const subs={dashboard:'Resumen general',reports:'Gráficos, tendencias y análisis',personal:'Gestión personal y familiar',trade:'Seguimiento de capital e inversiones',budgets:'Límites de gasto por categoría',goals:'Objetivos de ahorro con seguimiento de progreso',business:`Ingresos y gastos de tu ${prof.moduleLabel}`,purchases:'Compras de insumos y mercancía',inventory:'Control de stock de productos',settings:'Cuentas, vínculos e información del sistema'};
...
function renderSidebarNav(){
  ...
  NAV_DEFS.forEach(item=>{
    ...
    html+=`<button class="nav-item${state.currentPage===item.key?' active':''}" data-page="${item.key}" onclick="navigate('${item.key}')"><i class="ti ${item.icon}"></i> <span>${label}</span></button>`;
  });
  nav.innerHTML=html;
}
  ...
  document.getElementById('page-content').innerHTML=`
    <div class="hero-welcome">
      ...
      <div class="hero-content">
        <div class="hero-badge"><i class="ti ti-sparkles"></i> Gestión financiera sin complicaciones</div>
        <div class="hero-logo">Fin<span>Flow</span><span class="hero-ver">2.0</span></div>
