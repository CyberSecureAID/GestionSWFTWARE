...
function renderPage(){
  ...
  const titles={
    ...
    simple:'Gestión de capital interno',
    carta:'Carta del día'
  };
  const subs={
    ...
    simple:'Registra ingresos y gastos de forma rápida · control de capital personal',
    carta:'Punto de venta táctil para restaurantes y tiendas · genera facturas al instante'
  };
...
const NAV_DEFS=[
  ...
  /* Gestión de capital interno (antes Modo Simple): visible siempre */
  {key:'simple',label:'Gestión de capital interno',icon:'ti-table',section:'General'},
  /* Carta del día: visible siempre */
  {key:'carta',label:'Carta del día',icon:'ti-clipboard-list',section:'General'},
...
function renderSidebarNav(){
  ...
  NAV_DEFS.forEach(item=>{
    // Siempre visibles: simple, dashboard, reports, carta
    if(item.key!=='simple'&&item.key!=='dashboard'&&item.key!=='reports'&&item.key!=='carta'){
    ...
    const isCarta=item.key==='carta';
    const extraStyle=isCarta?'border:1px solid var(--amber);margin-bottom:4px;':'';
    const badge=isCarta
      ...
      :'';
    html+=`<button class="nav-item${state.currentPage===item.key?' active':''}" style="${extraStyle}" data-page="${item.key}" onclick="navigate('${item.key}')"><i class="ti ${item.icon}"></i> <span>${label}</span>${badge}</button>`;
  });
  nav.innerHTML=html;
}
