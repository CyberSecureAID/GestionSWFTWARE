/* ============================================================
   FinFlow — router.js  (versión reducida)
   Solo existen 3 páginas: welcome (portada), settings (configuración)
   y carta (Carta del día). Todo lo demás (dashboard, reportes,
   personal, trade, presupuestos, metas, negocio, compras, inventario)
...
function renderPage() {
  ...
  const subs = {
    settings: 'Respaldo local del navegador',
...
function renderSidebarNav() {
  ...
  nav.innerHTML = `
    ...
    <button class="nav-item${state.currentPage === 'settings' ? ' active' : ''}" data-page="settings" onclick="navigate('settings')"><i class="ti ti-settings"></i> <span>Configuración</span></button>
  `;
}
