...
function renderBusiness(){
  ...
  document.getElementById('topbar-actions').innerHTML=`<button class="btn success" onclick="openTxModal('business')"><i class="ti ti-plus"></i> Registrar ${isRestaurant?'venta o gasto':'movimiento'}</button>`;
  ...
  document.getElementById('page-body').innerHTML=`
    ...
    <div class="tx-list" id="tx-business">${txs.length?[...txs].sort((a,b)=>b.date>a.date?1:-1).map(txRow).join(''):`<div class="empty"><i class="ti ${prof.icon}"></i><p>Sin operaciones registradas. Registra tu primera venta o gasto de ${prof.moduleLabel} aquí.</p></div>`}</div>
  `;
}
