...
function renderPurchases(){
  ...
  document.getElementById('page-body').innerHTML=`
    ...
    <div class="tx-list">${txs.length?[...txs].sort((a,b)=>b.date>a.date?1:-1).map(txRow).join(''):`<div class="empty"><i class="ti ti-shopping-cart"></i><p>Sin compras registradas. Registra compras de ${isRestaurant?'insumos':'insumos o mercancía'} aquí, separado de los gastos operativos.</p></div>`}</div>
  `;
}
