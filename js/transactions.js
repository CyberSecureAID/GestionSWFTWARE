...
function txRow(t){
  ...
  return `<div class="tx-item">
    ...
    <div class="tx-actions">
      <button class="icon-btn edit" onclick="editTx('${t.id}')" title="Editar movimiento" aria-label="Editar movimiento"><i class="ti ti-pencil"></i></button>
      <button class="icon-btn" onclick="deleteTx('${t.id}')" title="Eliminar movimiento" aria-label="Eliminar movimiento"><i class="ti ti-trash"></i></button>
    </div>
  </div>`;
}
...
function openTxModal(module,editId){
  ...
  const invOptions=state.inventory.map(i=>`<option value="${i.id}">${esc(i.name)} (stock: ${i.stock} ${esc(i.unit||'uds')})</option>`).join('');
  ...
  document.getElementById('modal-body').innerHTML=`
    ...
    <div class="field"><label class="help-row">Descripción ${helpBtn('tx_desc')}</label><input type="text" id="f-desc" placeholder="Ej: Pago de nómina, Compra supermercado..." value="${esc(existing?(existing.desc||''):'')}"></div>
    ...
    ${state.accounts.length>1?`<div class="field"><label class="help-row">Cuenta ${helpBtn('tx_account')}</label><select id="f-account">${state.accounts.map(a=>`<option value="${a.id}" ${(existing?existing.accountId:state.activeAccount)===a.id?'selected':''}>${esc(a.name)}</option>`).join('')}</select></div>`:''}
    ${showInvLink?`
    ...
    `:''}
    <div class="field"><label class="help-row">Notas (opcional) ${helpBtn('tx_notes')}</label><textarea id="f-notes" placeholder="Información adicional...">${esc(existing?(existing.notes||''):'')}</textarea></div>
  `;
