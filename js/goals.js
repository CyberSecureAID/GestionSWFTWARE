...
function renderGoals(){
  ...
  document.getElementById('page-body').innerHTML=`
    ${state.goals.length?`<div class="goal-grid">${state.goals.map(goalCard).join('')}</div>`:moduleEmptyState({icon:'ti-target-arrow',title:'Crea tu primera meta de ahorro',text:'Ponte un objetivo, como unas vacaciones o un fondo de emergencia, y sigue tu progreso poco a poco.',btn:'Nueva meta',onclick:'openGoalModal()'})}
  `;
}
function goalCard(g){
  ...
  return `<div class="goal-card">
    <div class="goal-head">
      <div><div class="goal-name">${esc(g.name)}${done?' <span style="color:var(--green);font-size:11px;font-weight:600">✓ Completada</span>':''}</div>${g.deadline?`<div class="goal-deadline"><i class="ti ti-calendar" style="font-size:11px"></i> Meta: ${esc(g.deadline)}</div>`:''}</div>
      ${actionMenu('goal-'+g.id,[
        {label:'Agregar fondos',icon:'ti-plus',onclick:`addToGoal('${g.id}')`},
        {label:'Editar',icon:'ti-pencil',onclick:`editGoal('${g.id}')`},
        {label:'Eliminar',icon:'ti-trash',onclick:`deleteGoal('${g.id}')`,danger:true}
      ])}
    </div>
...
function editGoal(id){
  ...
  document.getElementById('modal-body').innerHTML=`
    <div class="field"><label class="help-row">Nombre de la meta ${helpBtn('goal_name')}</label><input type="text" id="f-goal-name" value="${esc(g.name)}"></div>
    <div class="field-row">
...
function addToGoal(id){
  ...
  document.getElementById('modal-body').innerHTML=`
    <div class="field-hint" style="margin-bottom:12px">${esc(g.name)} — actualmente ${fmt(g.current||0)} de ${fmt(g.target)}</div>
...
function deleteGoal(id){
  ...
  confirmDialog({
    title:'Eliminar meta',
    message:`¿Seguro que quieres eliminar la meta "${esc(g?g.name:'')}"? Esta acción no se puede deshacer.`,
    confirmLabel:'Eliminar',
    onConfirm:()=>{
      state.goals=state.goals.filter(x=>x.id!==id);
      showToast('Meta eliminada');renderPage();scheduleAutosave();
    }
  });
}
