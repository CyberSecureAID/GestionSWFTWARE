/* ============================================================
   FinFlow 2.0 — carta.js  v4
   "Carta del día" — punto de venta táctil universal para
   restaurantes, tiendas, y cualquier negocio.

   CORRECCIONES v4 (bugs reportados en runtime):
   - FIX: la barra de "ventas hoy / total del día" no tenía forma
     de reiniciarse. Se agrega botón "Reiniciar día" con confirmación,
     que limpia _cartaDaySales (no toca el catálogo de productos).
   - FIX: varios emojis de CARTA_EMOJIS se veían como un cuadrado
     (tofu) en ciertos sistemas/fuentes por ser glifos muy nuevos
     (Unicode 13+) o por llevar variation selectors innecesarios.
     Se reemplazó toda la paleta por emojis de soporte universal
     (Unicode ≤12, sin secuencias problemáticas).

   CORRECCIONES v3 (auditoría anterior, ya incluidas):
   ...
   - "Ventas de hoy" / "Total del día" / "Ganancias del día"
     filtran estrictamente por la fecha de HOY.
   - Historial de ventas se guarda por día (clave con fecha
     completa) con migración automática desde el formato viejo
     mensual si existiera.
   - Exportar/importar el catálogo de productos y el historial
     de ventas a CSV, diferenciado del export/import general.
   - Permite ingresar el costo de un producto directamente desde
     el modal de "Ganancias del día" cuando falta.
   ============================================================ */
...
/* ── Emojis de producto — paleta de soporte universal (Unicode ≤ 12) ──
   FIX bug "cuadrado vacío": se eliminaron glifos Unicode 13+ y secuencias
   con variation selector que no renderizan en fuentes de sistema antiguas
   (Windows 10 viejo, Android WebView viejo, algunas fuentes Linux):
   se quitaron 🫗 🫔 🫛 🫧 🪄 🪜 🪚 🪥 🪛 🛺 🪙 🫐 🫕 🥫 y variantes con ️ (VS16)
   como 🕶️ 🗜️ que dependían de la fuente para resolver el emoji-presentation. */
const CARTA_EMOJIS = [
  // Bebidas
  '🍺','🍻','🥤','🍹','🍸','🧉','☕','🫖','🧃','🥛','🍵','🧋','🍷','🥂','🍾',
...
/* ── FIX bug "no se puede reiniciar el día" ──
   Limpia únicamente el historial de ventas de HOY (no el catálogo de
   productos, no el nombre del negocio, no la configuración). Se usa
   cuando el usuario quiere arrancar de cero las métricas del día sin
   tener que esperar a que cambie la fecha del sistema. */
function cartaResetDay() {
  confirmDialog({
    title: 'Reiniciar día',
    message: `Esto borrará el registro de <b style="color:var(--text)">${_cartaDaySales.length} venta${_cartaDaySales.length!==1?'s':''}</b> de hoy y pondrá "ventas hoy" y "ganancias del día" en cero. El catálogo de productos NO se modifica. Esta acción no se puede deshacer.`,
    confirmLabel: 'Sí, reiniciar día',
    onConfirm: () => {
      _cartaDaySales = [];
      try { localStorage.removeItem(CK_SALES_DAY_PREFIX + cartaTodayKey()); } catch(e) {}
      showToast('Día reiniciado: ventas y ganancias en cero');
      renderCarta();
    }
  });
}
...
function renderCartaCatalog() {
  ...
  /* FIX: se agrega botón "Reiniciar día" (ti-refresh) dentro de la misma
     barra de stats, junto al botón de "Ver ganancias del día" que ya existía. */
  const statsRow = _cartaDaySales.length ? `
    <div class="carta-day-stats">
      ...
      <div class="carta-day-stats-actions">
        <button class="btn" onclick="cartaShowProfit()" style="border-color:var(--purple);color:var(--purple);font-size:11px;padding:6px 10px"><i class="ti ti-chart-line"></i> Ver ganancias del día</button>
        <button class="btn" onclick="cartaResetDay()" title="Borra el conteo de ventas y ganancias de hoy (no afecta el catálogo)" style="border-color:var(--red);color:var(--red-text);font-size:11px;padding:6px 10px"><i class="ti ti-refresh"></i> Reiniciar día</button>
      </div>
    </div>
  ` : '';
...
function cartaOpenSettings() {
  ...
  document.getElementById('modal-body').innerHTML = `
    ...
    <div class="field" style="margin-top:${_cartaBusinessLogo?'8px':'4px'}">
      <button class="btn" onclick="closeModal();cartaResetDay();" style="border-color:var(--red);color:var(--red-text);width:100%;justify-content:center"><i class="ti ti-refresh"></i> Reiniciar ventas y ganancias del día</button>
      <div class="field-hint">Borra solo el registro de ventas de hoy. El catálogo de productos no se modifica.</div>
    </div>
  `;
...
function cartaOpenDataMenu() {
  ...
  document.getElementById('modal-body').innerHTML = `
    ...
    <div style="display:flex;gap:8px;flex-wrap:wrap">
      ...
      <button class="btn" onclick="closeModal();cartaResetDay();" ${_cartaDaySales.length?'':'disabled'} style="border-color:var(--red);color:var(--red-text)"><i class="ti ti-refresh"></i> Reiniciar ventas de hoy</button>
    </div>
    <div class="field-hint" style="margin-top:14px">El catálogo importado <b style="color:var(--text2)">reemplaza</b> al actual. Las ventas de hoy se pueden exportar o reiniciar, pero no se importan (son tu registro).</div>
  `;
